...
#   define ListIterator(type) typename list<type>::iterator
#endif
...
namespace GameTutor
{
	...
	public:
		...
		void BeginBackTravel();
		element_type BackTravel();
		bool IsEndOfBackTravel();
	...
	//template<typename element_type> void GameTutor::CList<element_type>::AddItemFront(element_type ele)
	//{
	//	m_List->push_back(ele);
	//}
	...
	template<typename element_type> void GameTutor::CList<element_type>::BeginBackTravel()
	{
		m_CurrentIter = m_List->end();
		m_CurrentIter--;
		m_iTravelStepCounter = -1;
	}
	template<typename element_type> element_type GameTutor::CList<element_type>::BackTravel()
	{
		element_type re = (*m_CurrentIter);
		if (m_CurrentIter != m_List->begin())
		{
			m_CurrentIter--;
			m_iTravelStepCounter++;
		}
	...
	template<typename element_type> bool GameTutor::CList<element_type>::IsEndOfBackTravel()
	{
		return ((GetCount()==0) || (m_CurrentIter == m_List->begin()));
	}
}
