...
namespace GameTutor
{
	...
	{
		//Log("CControllerEventManager::OnEvent %d %d", Event.PointerData.X, Event.PointerData.Y);
		switch (Event.Type)
		{
		case ECE_POINTER:
			{
			
			Log("Truoc khi vo x %d y %d event %d id %d", Event.PointerData.X, Event.PointerData.Y, Event.PointerData.Event, Event.PointerData.ID);
			OnPointerEvent(Event.PointerData);			
			Log("Sau khi vo x %d y %d event %d id %d", Event.PointerData.X, Event.PointerData.Y, Event.PointerData.Event, Event.PointerData.ID);
			break;
			}
		case ECE_KEY:
	...
	{
		//Log("OnPointerEvent %d %d", Event.X, Event.Y);
		Log("Trong x %d y %d event %d id %d", Event.X, Event.Y, Event.Event, Event.ID);
		switch (Event.Event)
		{
		case EPE_PRESSED:
	...
	void CControllerPointerManager::Update()
	{
		...
		while (!m_PointersInfo.IsEndOfTravel())
		{
			...
			else if (info->Status == EPE_DRAGGED)
			{
				...
				if (info->DraggedDiffX<-10000)
					printf("haha\n");
				Log("Drag Position %d %d Drag Distance %d %d ",info->DraggedX, info->DraggedY, info->DraggedDiffX,info->DraggedDiffY);
				info->LastDraggedX = info->DraggedX;
				info->LastDraggedY = info->DraggedY;
				Log("Last Dragged %d %d ",info->LastDraggedX, info->LastDraggedY);
			}
	...
	bool CControllerPointerManager::GetActivePointer(__INT32 maxPointer, __INT32 &num, SPointerUserInfo* info)
	{
		bool isChange = false;
		...
		while ((!m_PointersInfo.IsEndOfTravel()) && (iNumberOfActiveTouch<=maxPointer))
		{
			...
			if (pointer->Status == EPE_DRAGGED)
			{
				...
				if (info[iNumberOfActiveTouch].DragDiffDistanceX<-1000)
					printf("huhu\n");
