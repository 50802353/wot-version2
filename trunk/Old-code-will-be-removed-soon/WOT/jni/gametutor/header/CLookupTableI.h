...
namespace GameTutor
{
	...
	public:
		...
		void BeginTravel();
		element_type Travel();
		element_type Travel(__INT32 &key);
		element_type RemoveCurrentTravel();
		bool IsEndOfTravel();
	protected:
	...
	template<typename element_type> element_type GameTutor::CLutI<element_type>::RemoveItem(__INT32 key)
	{
		...
		{
			typename map<__INT32, element_type, ltstr>::iterator cur  = m_List.find(key);	
			m_List.erase(key);
			return re;
		}
		else
	...
	template<typename element_type> void GameTutor::CLutI<element_type>::DeallocateElementPointer()
	{
		if (m_List.size() > 0)
		{
			typename map<__INT32, element_type, ltstr>::iterator cur  = m_List.begin();
	...
	template<typename element_type> element_type GameTutor::CLutI<element_type>::Travel(__INT32 &key)
	{
		element_type re = (*m_CurrentIter).second;
		key = (*m_CurrentIter).first;
		if (m_CurrentIter != m_List.end())
		{
			m_CurrentIter++;
		}
	...
	template<typename element_type> element_type GameTutor::CLutI<element_type>::RemoveCurrentTravel()
	{
		element_type re;
		m_CurrentIter--;
		__INT32 key = (*m_CurrentIter).first;
		m_CurrentIter++;
		if (GetElement(key, re))
		{
			typename map<__INT32, element_type, ltstr>::iterator cur  = m_List.find(key);	
