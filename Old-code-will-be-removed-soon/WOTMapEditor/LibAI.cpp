...
bool AStar::getNextCell(const cell &current,const int &index,cell &next)
{
	...
	*/
	...
	if( i < 0 || i>= N_HEIGHT || j< 0 || j >= N_WIDTH)
		return false;
	if( m_matrix[i*N_WIDTH + j] <= KET_THUC) 
	{
		if (iNext[index] * jNext[index] != 0)
			if (m_matrix[current.i*N_WIDTH + j] > KET_THUC || m_matrix[i*N_WIDTH+current.j] > KET_THUC)
				return false;
		next = cell(i,j);
		return true;
	}
...
bool AStar::pathFinder(bool isStep, CList<cell> *lst, int Starti, int Startj, int Endi, int Endj)
{
	...
	while(dMatrix.empty() == false)
	{
		...
		m_matrix[currentCell.i * N_WIDTH + currentCell.j] = ENTERED;
		...
		for(int i = 0;i< 8 ; ++i)
		{
			...
			if ( getNextCell(currentCell,i,nextCell))
			{
				...
				///if(m_State.find(nextCell) == --m_State.end())
				{
					...
					// Luu tr? tr?ng thái ô trong ma tr?n
					m_matrix[nextCell.i * N_WIDTH + nextCell.j] = ENTERING;
...
void AStar::setPath(bool isSet, CList<cell> *lst)
{
	...
	m_matrix[current.i * N_WIDTH + current.j] = KET_THUC;
	lst->AddItem(current);
	while(current != start)
	{
		current = m_State[current].prev;
		// set value cho current
		m_matrix[current.i * N_WIDTH + current.j] = value;

		lst->AddItem(current);
	}
	m_matrix[current.i * N_WIDTH + current.j] = BAT_DAU;
	...
	// kh?i d?ng l?i các giá tr? ban d?u + v?t c?n:
		for(int i=0;i<N_HEIGHT;++i)
			for(int j=0;j<N_WIDTH;++j)
				if(m_matrix[i*N_WIDTH + j] < 0)
					m_matrix[i*N_WIDTH + j] = EMPTY;
		// T?o l?i begin -	end
		backupStart = m_matrix[iStart*N_WIDTH + jStart];
		m_matrix[iStart*N_WIDTH + jStart] = BAT_DAU;
		backupEnd = m_matrix[iEnd*N_WIDTH + jEnd];
		m_matrix[iEnd*N_WIDTH + jEnd] = KET_THUC;
}
...
void AStar::Restore() {
	...
	m_matrix[iStart*N_WIDTH + jStart] = backupStart;
	m_matrix[iEnd*N_WIDTH + jEnd] = backupEnd;
}
