	...
	void CControllerPointerManager::GetActivePointer(__INT32 maxPointer, __INT32 &num, SPointerUserInfo* info)
	{
		...
		while ((!m_PointersInfo.IsEndOfTravel()) && (iNumberOfActiveTouch<=maxPointer))
		{
			...
			if (pointer->Status == EPE_DRAGGED)
			{
				...
				info[iNumberOfActiveTouch].OriginalPosition_X = pointer->PressedX;
				info[iNumberOfActiveTouch].OriginalPosition_Y = pointer->PressedY;
				info[iNumberOfActiveTouch].CurrentPosition_X = pointer->DraggedX;
				info[iNumberOfActiveTouch].CurrentPosition_Y = pointer->DraggedY;
			...
			else if (pointer->Status == EPE_PRESSED)
			{
				...
				info[iNumberOfActiveTouch].OriginalPosition_X = pointer->PressedX;
				info[iNumberOfActiveTouch].OriginalPosition_Y = pointer->PressedY;
				info[iNumberOfActiveTouch].CurrentPosition_X = pointer->PressedX;
				info[iNumberOfActiveTouch].CurrentPosition_Y = pointer->PressedY;
